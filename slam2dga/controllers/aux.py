...
class EpsilonGreedyAgent:
    ...
    def __init__(self, epsilon, num_bandits):
        ...
        self.est_values = [0] * num_bandits  # Estimated value of each bandit
        self.chosen_count = [0] * num_bandits  # Count of times each bandit is chosen

    # Choose action
    def choose_action(self):
        if random.random() < self.epsilon:
            return random.randint(0, self.num_bandits - 1)  # Explore
        else:
            max_value = max(self.est_values)
            max_indices = [i for i, x in enumerate(self.est_values) if x == max_value]
            return random.choice(max_indices)

    # Update the estimated values based on the reward
    def update_estimation(self, chosen_bandit, reward):
        self.chosen_count[chosen_bandit] += 1
        n = self.chosen_count[chosen_bandit]
        value = self.est_values[chosen_bandit]
        # Incremental calculation of average
        self.est_values[chosen_bandit] += (reward - value) / n
    def getState(self):
        return self.est_values

    def setState(self, state):
        self.est_values =state
