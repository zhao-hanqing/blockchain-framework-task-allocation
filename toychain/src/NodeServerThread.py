from toychain.src.Block import TaskNode, TaskTree
import dill as pickle
from toychain.src.wire import to_wire, from_wire
import threading
...
class NodeServerThread(threading.Thread):
    ...
    def handle_connection(self, sock):
        ...
        # Receive request
        data = sock.recv(655360)
        request = from_wire(pickle.loads(data))
        #request = pickle.loads(data)
        #print("this req data: ", request)

        # Send the answer
        answer = self.message_handler.handle_request(request)
        #print("this req answer: ", answer)
        safe = to_wire(answer)
        self.send(pickle.dumps(safe), sock)
    ...
    def send_request(self, enode, request):
        ...
        try:
            ...
            self.send(pickle.dumps(to_wire(request)), sock)
        except Exception as e:
        ...
        try:
            answer = from_wire(pickle.loads(data))  # decode after receive
